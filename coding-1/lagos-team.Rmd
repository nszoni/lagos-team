---
title: "Lagos team - C1: Team Project"
author: "Viktoria, Sara, Ali, Benjamin, Son"
output: 
  pdf_document:
...
pacman::p_load(tidyverse, dplyr, ggplot2, janitor, modelsummary, RCurl,
               kableExtra, vtable)

# load data

x <- getURL('https://raw.githubusercontent.com/nszoni/lagos-team/main/coding-1/data/Data_DA1_HW1_Lagos.csv')
df <- read.csv2(text = x, encoding = 'latin1')
...
df[df=='Coca Cola, 0.5l plastic bottle']<-'Coke'
df[df=='Snickers bar (50g)']<-'Snickers'

df$longitude <- as.numeric(df$longitude)
df$latitude <- as.numeric(df$latitude)
df$google_rating <- as.numeric(df$google_rating)
df$number_of_reviews <- as.integer(df$number_of_reviews)
df$collection_date <- as.POSIXlt(df$collection_date)
df$is_discounted <- as.factor(df$is_discounted)
df$has_website <- as.factor(df$has_website)
...
df %>% 
  group_by(item_name) %>%
      datasummary(data=., price_huf ~  Mean + Median + SD + Min + Max + P25 + P75 + N,  output = 'kableExtra')
...
st(df,
   vars = c('price_huf', 'google_rating', 'number_of_reviews'),
   summ=c('NA(x)',
          'mean(x)',
          'median(x)',
          'sd(x)',
...
datasummary( (`Online` = price ) + (`Retail` = price_online ) + (`Price difference` = p_diff ) ~
