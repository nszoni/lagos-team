## DE1 - Movies Analysis (Term 2 Project)

## Table of Contents  
1. [Overview](#Overview)  
2. [Data Sources](#Data_sources)  
3. [KNIME Workflow](#Knime)  
4. [Analytical questions](#Analytical_questions)  
...
Our `MySQL` instance was hosted on AWS’ `RDS service` (under a Free Tier subscription with the smallest virtual machine). For setting up connection, we had allowed every incoming `IP addresses` through the firewall when defining `Security Groups`.
...
A `csv` file containing the TMDb movie_id-s and the titles of the movies was read from our [external files folder](https://github.com/nszoni/lagos-team/tree/main/de-1/term2/external_files) with a `File Reader` node.
...
Hover             |  Over
:-------------------------:|:-------------------------:
![](images/sunburst1.png)  |  ![](images/sunburst2.png)
